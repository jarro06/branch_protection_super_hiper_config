...
def job_config = [
    job: [
        name: "branch_protection_super_hiper_Pipeline_develop"
    ],
    git: [ 
        branch: "develop"
    ]
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "branch_protection_super_hiper_Pipeline_develop", "develop")
                }
            }    
        }
        stage('Build') {
            steps {
                echo "Build"
                script {
                    dir ("branch_protection_super_hiper") {
                        jslMaven.build()
                    }
                }
            }    
